...
def transform(mean, std, rotation):
    if rotation != 0.0:
...
def split(mean=0.1311, std=0.3081, rotation=0.0):
    train_transform, test_transform = transform(mean, std, rotation)
...
def load(mean=0.1311, std=0.3081, rotation=0.0):
    seed = 1

    train, test = split(mean, std, rotation)
